...
# ╔═╡ 26452b45-dbbf-4857-9b06-e24de6fd2663
function constraint_equations(position, parameters)
    x_block, y_block, bar_angle = position
    bar_length = parameters.bar_length

    x_bar = x_block + bar_length * sin(bar_angle)
    y_bar = y_block - bar_length * cos(bar_angle)
...
# ╔═╡ 289b3771-5e53-49cb-ba3a-52d05d40bc06
function mass_matrix(parameters)
    return Diagonal([
        parameters.mass_block,
        parameters.mass_block,
        parameters.inertia_bar
    ])
end

# ╔═╡ 2c961bb8-5cbb-40ec-b8b5-91496cfa2af4
function applied_forces(position, velocity, parameters)
    x_block, _, bar_angle = position
    _, _, bar_angular_velocity = velocity

    spring_force = -parameters.spring_constant * x_block
    gravity_force = -parameters.mass_block * parameters.gravity

    bar_gravity_torque = -parameters.mass_bar *
        parameters.gravity * parameters.bar_length / 2 * sin(bar_angle)
...
# ╔═╡ e162fda1-badb-4b63-9c6f-e89c00b7e5b8
function equations_of_motion!(du, u, parameters, time)
    position = u[1:3]
    velocity = u[4:6]

    M = mass_matrix(parameters)
    Q = applied_forces(position, velocity, parameters)

    Φq = [
        0 1 0;
        -1 0 parameters.bar_length * cos(position[3]);
        0 -1 parameters.bar_length * sin(position[3])
    ]
...
# ╔═╡ 48787a69-4b8c-4b65-8315-979d8017368a
begin
	parameters = (
	    mass_block = 2.0,
	    mass_bar = 1.0,
	...
	initial_state = [
	    0.2, 0.0, 0.4,   # position
	    0.0, 0.0, 0.0    # velocity
	]
	
	problem = ODEProblem(equations_of_motion!, initial_state, (0.0, 10.0), parameters)
	solution = solve(problem, Rodas5())
end

# ╔═╡ 583a0de8-29b5-4946-865d-498ea4b73bae
begin
	x_block = solution[1, :]
	bar_angle = solution[3, :]
	
	x_bar = x_block .+ parameters.bar_length .* sin.(bar_angle)
	y_bar = -parameters.bar_length .* cos.(bar_angle)
	
	plot(x_block, zeros(length(x_block)), label="Block path")
	plot!(x_bar, y_bar, label="Bar tip path")
end

# ╔═╡ c2a7910e-e592-4cfb-82da-b1dfa0c57117
constraint_forces = [
    mass_matrix(parameters) *
    solution[4:6, i]
...
git-tree-sha1 = "f4a81fe4d30c68d9dc1f8a9b7193525ec19a7320"
