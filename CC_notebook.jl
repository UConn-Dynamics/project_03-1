...
# ╔═╡ de5d804b-9248-4f5a-9fc2-c93c6cade3eb
### Import packages

begin
    using Symbolics, Plots, NonlinearSolve, 
          DifferentialEquations, LinearAlgebra, Latexify
end

# ╔═╡ b2870eba-42ea-4b0f-84b2-9f9cd89e2efd
### Use MTK time variableD
...
# ╔═╡ 8d9d1898-c49b-436e-8ca1-a5319685b0e2
### Physical constants
...
begin
    ...
    g   = 9.81        # gravity
    I1  = 0.001       # block moment of inertia
    I2  = (1/12)*m2*L^2  # bar moment of inertia
end

# ╔═╡ 1ae92915-da0e-441c-879a-c4729097129b
### Pin location on the block and bar

begin
    s1A = [0.0,  0.0]   # pin at block 
    s2A = [-L/2, 0.0]   # pin at bar 
end

# ╔═╡ 529b9596-579d-4757-ac57-27f9f74a130a
#=
Generalized coordinates: q = [x1, y1, θ1, x2, y2, θ2] where (x1,y1,θ1) describes the sliding block and (x2,y2,θ2) describes the rigid bar
=#

begin
    q  = @variables x1(t) y1(t) θ1(t) x2(t) y2(t) θ2(t)
    qt = @variables dx1(t) dy1(t) dθ1(t) dx2(t) dy2(t) dθ2(t)
    q  = collect(q)
    qt = collect(qt)
end

# ╔═╡ f93ca508-a1ab-427f-bb72-befd4001bedb
### Rotation matrix

function A_matrix(theta)
    [cos(theta)  -sin(theta)
     sin(theta)   cos(theta)]
end

# ╔═╡ c5b7b886-9a0e-46c4-aab7-efbebcce8374
#= 
Constraint equations C(q, t):
Four constraints:
     C1: y1 = 0         
     C2: θ1 = 0         
     C3: pin joint x -> r1 + A1s1^A - r2 - A2s2^A = 0  
     C4: pin joint y -> r1 + A1s1^A - r2 - A2s2^A = 0
=#
...
# ╔═╡ eb42b48c-cbce-4200-83ae-009e699de06b
### Constraint jacobian Cq(q, t)

begin
    function Cq(q, t)
        Symbolics.jacobian(C(q, t), q)
    end
...
# ╔═╡ 355d067a-4330-4557-a3b1-41a453ef7ce2
### Compute constrained acceleration 

begin
    function Qd(q, dq, t)
        Cqdq = Cq(q, t) * dq                         
        dCqdq_dq = Symbolics.jacobian(Cqdq, q)       
        -dCqdq_dq * dq                                 
    end

    Symbolics.simplify.(Qd(q, qt, t))
    Qd_fun = Symbolics.build_function(Qd(q, qt, t), q, qt, t;
expression=Val(false))[1]
end

# ╔═╡ 71596cbb-2427-44a5-a02f-adc1d06b071e
### Mass matrix

M_mat = Matrix(Diagonal([m1, m1, I1, m2, m2, I2]))

# ╔═╡ 1bb5e2a4-2f69-4378-a72c-5ab81d2cc5d7
### Constructs augmented system
...
# ╔═╡ dc29a55b-54f6-4550-9fe3-76952f846592
### Generalized forces 

begin
    @variables x1_s θ2_s
    F_spring_sym = -k*x1_s
    τ_bar_sym    = -m2*g*(L/2)*cos(θ2_s)
	

    F_spring_fun = Symbolics.build_function(F_spring_sym, x1_s;						expression=Val(false))
    τ_bar_fun    = Symbolics.build_function(τ_bar_sym,    θ2_s; expression=Val(false))
end

# ╔═╡ 23adf600-201e-44c3-b292-428f491794a6
...
# ╔═╡ 0be28805-d4a1-4a85-a342-002c2eadd745
### Forcing terms

function RHS(q, dq, t)
    [Qe(q, t); Qd_fun(q, dq, t)]
end

# ╔═╡ cbae42d7-f69d-45a6-ae6a-a42ffc5a136d
### Baumgarte stabilization

begin
    function setup_q0(q_init, p)
        th2_fixed = p[1]
    ...
    q_prob = NonlinearProblem(setup_q0, q_guess, [th0])
    q0_sol = solve(q_prob, NewtonRaphson())
    q0     = q0_sol.u
    dq0    = zeros(6)
    u0     = vcat(q0, dq0)
    println("Initial conditions: ", q0)
end

# ╔═╡ 8db8e21a-7cb6-4435-8961-406ca4530ab0
### ODE function using augmented method 

function dynamics!(du, u, params, time)
    q  = u[1:6]
    dq = u[7:12]
    a, B = params

    rhs     = RHS(q, dq, time)
    rhs[7:10] += -2*a*(Cq_fun(q, time)*dq) - B^2*C_fun(q, time)

    sol_aug  = LHS(q, time) \ rhs
    du[1:6]  = dq
    du[7:12] = sol_aug[1:6]
...
# ╔═╡ fbd1e42b-8dca-458c-97ec-bff4fb39e31f
### Solve ODE

begin
    tspan   = (0.0, 10.0)
    params  = (1.0, 1.0)     
    problem = ODEProblem(dynamics!, u0, tspan, params)
    solution = solve(problem, Tsit5(), abstol=1e-8, reltol=1e-8)
    t_steps  = LinRange(0.0, 10.0, 300)
end

# ╔═╡ c8530abd-0b9f-45eb-a9a8-feef97c98e6b
### Plot block position x1
...
begin
    plot(solution(t_steps).t,
         ...
         linewidth=2,
         xlabel="Time (s)",
         ylabel="Block Position x1 (m)",
         label="x1",
         title="Block Horizontal Position")
end

# ╔═╡ 661bff6a-486b-4eb9-a737-45a8d81d84a4
### Plot bar angle θ2

begin
    plot(solution(t_steps).t,
         solution(t_steps)[6, :] .* (180/π),
         linewidth=2,
         xlabel="Time (s)",
         ylabel="Angle θ2 (deg)",
         label="θ2",
...
#### Constraint forces at each time step
...
begin
    ...
    for i in 1:length(t_steps)
        ...
        dqi = solution(t_steps[i])[7:12]

        rhs = RHS(qi, dqi, t_steps[i])
        rhs[7:10] += -2*a_b*(Cq_fun(qi, t_steps[i])*dqi) - 
                      B_b^2*C_fun(qi, t_steps[i])

        sol_aug = LHS(qi, t_steps[i]) \ rhs
        R[i, :] = sol_aug[7:10]  
    end
end
...
begin
    plot(t_steps, R[:, 1], label="C1  (y₁ normal)",   linewidth=2)
    plot!(t_steps, R[:, 2], label="C2  (θ₁ reaction)", linewidth=2)
    plot!(t_steps, R[:, 3], label="C3  (pin x)",        linewidth=2)
    plot!(t_steps, R[:, 4], label="C4  (pin y)",        linewidth=2,
          xlabel="Time (s)", ylabel="Constraint Force (N)",
          title="Lagrange Multipliers (Constraint Forces)")
end

# ╔═╡ 73a839af-b588-4808-a6c2-564fa46f046f
begin
    q_vals  = solution(t_steps)[1:6,  :]
    dq_vals = solution(t_steps)[7:12, :]
    force_scale = 0.02

    anim = @animate for i in 1:length(t_steps)
        x1  = q_vals[1, i]
        th2 = q_vals[6, i]

        pivot = [x1, 0.0]
        tip   = [x1 + L*cos(th2), L*sin(th2)]

        # Base plot
        plot(xlim=(-1.2, 1.2), ylim=(-0.6, 0.7),
             aspect_ratio=:equal, legend=:topright,
             title="t = $(round(t_steps[i], digits=2)) s")

        # Ground line
        plot!([-1.2, 1.2], [0, 0], lw=3, color=:black, label="")

        # Spring (wall to block)
        plot!([-0.6, x1], [0, 0], lw=2, color=:blue, label="spring")

        # Block
        scatter!([x1], [0], markersize=12, color=:green, label="block")

        # Bar
        plot!([pivot[1], tip[1]], [pivot[2], tip[2]],
              lw=4, color=:purple, label="bar")

        # Bar tip
        scatter!([tip[1]], [tip[2]], markersize=6, color=:orange, label="")

        # Pin joint
        scatter!([pivot[1]], [pivot[2]], markersize=5, color=:red, label="pin")

        # Constraint force arrow at pin
        quiver!([pivot[1]], [pivot[2]],
                quiver=([force_scale * R[i, 3]], [force_scale * R[i, 4]]),
                color=:red, linewidth=2, label="F_pin")

    end  # ← closes @animate for
...
manifest_format = "2.0"
project_hash = "e8028402c687be5ec33bb1de3adf429996c45a9c"
...
[[deps.DelayDiffEq]]
deps = ["ArrayInterface", "DataStructures", "DiffEqBase", "FastBroadcast", "ForwardDiff", "LinearAlgebra", "Logging", "OrdinaryDiffEqCore", "OrdinaryDiffEqDefault", "OrdinaryDiffEqDifferentiation", "OrdinaryDiffEqFunctionMap", "OrdinaryDiffEqNonlinearSolve", "OrdinaryDiffEqRosenbrock", "RecursiveArrayTools", "Reexport", "SciMLBase", "SciMLLogging", "SimpleNonlinearSolve", "SymbolicIndexingInterface"]
git-tree-sha1 = "faae8de6486c457d149e35b1b063281a4e17942c"
uuid = "bcd4f6db-9728-5f36-b5f7-82caef46ccdb"
version = "5.71.0"
weakdeps = ["DiffEqNoiseProcess", "Random", "StochasticDiffEqCore"]

    [deps.DelayDiffEq.extensions]
    DelayDiffEqStochasticDiffEqCoreExt = ["StochasticDiffEqCore", "DiffEqNoiseProcess", "Random"]
...
git-tree-sha1 = "01eb732749884b8c481fc165c62c0b9652fd673a"
...
git-tree-sha1 = "eaa4e9f9db4f055a15fad765156707c9467aa92d"
...
git-tree-sha1 = "4e38e3b47192bcf3568e52f6a97905875b0b8e8e"
...
git-tree-sha1 = "3866a9d344eaee469d764595d4e18d9517f8e631"
...
git-tree-sha1 = "a402de6bb134328d23db0a1ca4151b1f6d4cee88"
...
git-tree-sha1 = "0e5d2d2ad11f32dbd06b69095405398e3d2ecacb"
...
git-tree-sha1 = "78edf37883a3524b2c1ea20b039a1040d3c0a4df"
...
git-tree-sha1 = "1c7563f823d75bf979c0578296d50ff3e268b043"
...
git-tree-sha1 = "a3d7837832e515111c95a02df7dc55edbdf17d8a"
...
git-tree-sha1 = "922db5c9c2ac78c68ce62edffefc61a3add303a7"
...
git-tree-sha1 = "65e432f49eae999e39d4f137e6a7527eb70ff2c5"
...
git-tree-sha1 = "9bc7b74e950dfdc8dac95280b7d8fd49aa8fb7bc"
...
git-tree-sha1 = "9f4231321117408aa9b7884d5379b646f8d07754"
...
git-tree-sha1 = "8d82ed0b5114eac176ce29237b5b12c399517ff8"
...
git-tree-sha1 = "c2afe0e4e301f785a7447dbb6b7111fdabbd0b44"
...
git-tree-sha1 = "30d557d0cd853356b7a3926cc069f82fe97f2af0"
uuid = "aefaaa88-39f2-4e89-b162-d61b7e8cc81b"
version = "1.0.0"

[[deps.StochasticDiffEqLevyArea]]
deps = ["LinearAlgebra", "Random", "SpecialFunctions"]
git-tree-sha1 = "ac6b47de2be99255ba42e2b30a86a927ac53b1c7"
uuid = "90dbc90e-856a-4131-af2c-e8c5aa0f35b5"
version = "1.0.0"

[[deps.StochasticDiffEqLowOrder]]
deps = ["DiffEqBase", "DiffEqNoiseProcess", "LinearAlgebra", "MuladdMacro", "OrdinaryDiffEqCore", "RecursiveArrayTools", "Reexport", "SciMLBase", "StaticArrays", "StochasticDiffEqCore"]
git-tree-sha1 = "a32b3996548663ab20c6e0d0fbcef332bf42400e"
uuid = "d15fe365-ce7f-450a-828a-7985bd5b681b"
version = "1.0.0"

[[deps.StochasticDiffEqMilstein]]
deps = ["DiffEqBase", "DiffEqNoiseProcess", "LevyArea", "LinearAlgebra", "MuladdMacro", "OrdinaryDiffEqCore", "RecursiveArrayTools", "Reexport", "SciMLBase", "StaticArrays", "StochasticDiffEqCore"]
git-tree-sha1 = "fda1b680e65fa5a16bb2c7bc0da0c2cd6d0f95bf"
...
git-tree-sha1 = "7b5f6d575cfc518a5b324e33609ebcc3d679cc6d"
uuid = "db241ea8-0e6b-4abc-8f2d-1adff2294fd9"
version = "1.0.0"

[[deps.StochasticDiffEqRODE]]
deps = ["DiffEqBase", "LinearAlgebra", "MuladdMacro", "OrdinaryDiffEqCore", "RecursiveArrayTools", "Reexport", "SciMLBase", "StaticArrays", "StochasticDiffEqCore"]
git-tree-sha1 = "51ba8a4d199a8e386c578d974a2e453c7f7b18a7"
...
git-tree-sha1 = "60d6d7c180aeae58f254d0c5d3377bc4f6bb959d"
...
git-tree-sha1 = "b214b89e4b384ca27561c34aa0169da0de29977d"
...
git-tree-sha1 = "e55b5c8e019d079a8311900c5a3ff513b509de6a"
